...
def homepage_view(request, app_menu):
    ...
    if app_menu_cleaned == "Products" or app_menu_cleaned == "Home":
    ...
    testimonial_data = get_testimonials_view(request)
    # print(f'ttttttttttttttttt{testimonial_data=}')

    

    Popup_response = PopupImage_view(request, app_menu_cleaned)
    # print(f'{Popup_response=}')

    Popup_data = Popup_response
    # print(f'======{Popup_data=}')
...
def Add_address_view(request):
    ...
    return render(request, "add_address.html",{"user_data" : request.session.get("user_data"),'cart_count':cart_count})
...
def get_testimonials_view(request):
    ...
    try:
        response = requests.get(f"{Baseurl}/testimonials/active/")
        if response.status_code == 200:
            data = response.json()
            testimonial_data = data.get("data", [])
            # print(f'{testimonial_data=}')
...
def Add_testimonials_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            response = requests.post(
                f"{Baseurl}/user/add-testimonial/",
                json=payload,
                headers=headers
            )
            ...
            if response.status_code == 201:
                data = response.json()
                success_msg = data.get("message", "Submitted")

                # ❗ Re-fetch updated testimonials
                active_testimonial_data = get_testimonials_view(request)

                return render(request, 'add_testimonial.html', {
                    "cart_count": cart_count,
                    "user_data": user_data,
                    "success_msg": success_msg,
                    "active_testimonial_data": active_testimonial_data
                })

        except Exception as e:
            return HttpResponse(f"❌ Exception occurred: {e}")

    # ✅ GET request — you must fetch testimonial_data here too
    active_testimonial_data = active_testimonials_view(request)
    # print(f'{active_testimonial_data=}')
    return render(request, 'add_testimonial.html', {
        "cart_count": cart_count,
        "user_data": user_data,
        "active_testimonial_data": active_testimonial_data
...
'''===============================  Active Testimonail Method ============================================'''

def active_testimonials_view(request):
    access_token = request.session.get("access_token")
    user_data = request.session.get("user_data")
    cart_count = request.session.get("cart_count")
  
    active_testimonial_data = []
    ...
    try:
        response = requests.get(f"{Baseurl}/user/testimonials/active/",headers=headers)
        # print(f'{response=}')
        # print(f'{response.text=}')
        if response.status_code == 200:
            data = response.json()
            # print(f'{data=}')
            active_testimonial_data = data.get("data", [])
            # print(f'{active_testimonial_data=}')
            return active_testimonial_data
...
def get_notification_view(request):
    ...
    try:
        response = requests.post(f"{Baseurl}/get-notifications/", headers=headers)
        print(f"{response=}")
        print(f"{response.text=}")
        if response.status_code == 200:
            data = response.json()
            ...
            print(f'{notification_data=}')
            # return HttpResponse(f'{notification_data=}')
...
#         if not access_token or not notification_id:
#             return JsonResponse({'error': 'Unauthorized or missing ID'}, status=400)
...
#         # print(f'delete notification =={response=}') 
...
def delete_notification_view(request):
    if request.method == 'POST':
        ...
        try:
            response = requests.post(  # If DELETE fails, use POST
                f"{Baseurl}/delete-notification",
                headers=headers,
                json={'notification_id': notification_id}
            )
            ...
            try:
                ...
                return JsonResponse({'error': 'Invalid JSON from API', 'details': response.text}, status=500)
